...
describe("U3R: DAI->USDC", function () {
  ...
  it("Swap exact DAI (4000) for USDC via pool 0.05% - relayed", async function () {
    ...
    const swapParams = ethers.utils.defaultAbiCoder.encode(
      ["tuple(uint256, uint256, uint256,  uint256, address,  uint160, bool)"],
      [[amountIn, minOutInUSDC, deadline, curr_nonce,  pool,    sqrtPriceLim,  exactIn]]);
    const callbackData = ethers.utils.defaultAbiCoder.encode(
      ["tuple(address, address, address, uint24)"],
      [[DAI,       USDC,      dest,      fees]]);
    const full_payload = ethers.utils.defaultAbiCoder.encode(["bytes", "bytes"], [swapParams, callbackData]);
    ...
    // -- returned value --
    const USDC_received = await U3R.connect(owner).callStatic.relayedSwap(sig.v, sig.r, sig.s, full_payload);
